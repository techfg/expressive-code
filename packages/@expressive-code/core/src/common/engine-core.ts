...
export interface ExpressiveCodeEngineConfig {
	 ...
	 *
	 * Defaults to the `github-dark` and `github-light` themes when using `ExpressiveCodeEngine`. When
	 * using `ExpressiveCodeEngineCore`, you must provide at least one theme.
	 */
...
/**
 * The Expressive Code engine coreis responsible for rendering code blocks to a
 ...
 */
export class ExpressiveCodeEngineCore implements ResolvedExpressiveCodeEngineConfig {
	/**
	 * Creates a new instance of the Expressive Code engine core.
	 *
	...
	constructor(config: ExpressiveCodeEngineConfig) {
		...
		this.themes = Array.isArray(config.themes) ? config.themes : config.themes ? [config.themes] : []
